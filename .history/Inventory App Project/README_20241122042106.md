...
the way we work with data, both storing it and manipulating it in this project show competency with algorithms, and data structures. from building the JSON objects and storing them in an array, to using ```.map()``` or ```forEach``` to iterating and manipulate that data. 
Also the use of filtering when or searching algorithms to return specific data shows solid understand of these principles
