...
the way we work with data, both storing it and manipulating it in this project show competency with algorithms, and data structures. from building the JSON objects and storing them in an array, to using ```.map()``` or ```forEach``` to interating and manipulate that data
